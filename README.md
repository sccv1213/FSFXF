...
1. **打开视频**(Ctrl+O),默认三格布局。
2. **布局按片段生效**:左栏选 三格/两格/四格/无网格;选择模式拖分界线微调,左右边缘线 = 裁剪。
3. **画补丁**:目标模式画脏区域(红)自动生成源区域(绿);源模式反向;锁定对齐自动按同一相对位置重算。
4. **复制格子**:整格复制,来源格/目标格多选互斥,每条规则可勾选"左右翻转"。
5. **分段**:播放头移到变化点 → 工具栏"分割",每段独立布局/启停补丁与复制。
6. **预览/渲染**:预览当前帧左右并排对比;添加队列 → 开始处理,输出 `原名_修复.mp4`。
7. **模板/批量**:保存或应用模板;无分段工程可批量导入多个视频处理。

播放:空格播放/暂停,←/→ 跳 1 秒(Ctrl=15 秒,Shift=30 秒),方向键悬停画面时微调补丁/分界线,0.5–5 倍速。

## 画质策略

| 项 | 默认 | 说明 |
|---|---|---|
| 编码器 | 硬件优先 NVENC | 50 系显卡同码率画质与 x264 medium 无肉眼差异;失败时弹窗三选一(软件重试/跳过/取消),不自动切换 |
...
- **保存模板**:把当前时间所在分段的补丁/复制/边缘线/分界线保存到 `templates/<名>.fstpl.json`。
- **应用模板**:替换当前分段(全局模式 = 全局)的补丁/复制/网格。
- **批量导入视频**:当前工程无分段时,按当前补丁/复制/网格/设置逐个处理多个视频。
...
| 空格 | 播放/暂停 |
| ← / → | 后退 / 前进 1 秒(Ctrl = 15 秒,Shift = 30 秒) |
| ←/→/↑/↓ | 选择模式:指向补丁框 ±1 像素微调(Shift = ±10);指向分界/边缘线移线 |
| Esc | 取消当前绘制 |
...
CI:`.github/workflows/ci.yml` 在 Windows runner 上安装 Python 依赖与 ffmpeg/ffprobe 后运行全量测试。

架构、关键设计决策、与旧版差异、已知错误与修复记录见文末《详细设计文档》。

---

## 详细设计文档(原 CLAUDE.md)

> 本部分供维护者参考,保留原 CLAUDE.md 的架构、关键设计决策与修复记录。
...
│   │                     #   + 全部模型变换(split_at/toggle/set_crop/realign)+ JSON v3
...
└── ui/
    ├── main_window.py    # 仅 UI 组装 + 信号接线(约 700 行,旧版 1269)
...
19. **失败处理原则（用户明确要求）**：不自动切换/不静默兜底——NVENC 失败弹窗三选一（软件重试/跳过/取消全部）；音频输出失败明示"预览将无声音"（状态栏橙色徽标）；旧工程文件(version<2 且无迁移路径)显式拒绝并弹"请重建"。
20. **UI 布局坑**：QHBoxLayout 行尾必须 addStretch(1)（剩余空间平均拉宽）；播放按钮纯文字不用 emoji（Windows 撑高按钮）；QColor 必须传对象（6.11 不接受 str）；QAudioSink 无 write()（sink.start() 返回 QIODevice）、reset() 置 Stopped 后必须立即 start() 重启（无声根因）、缓冲仅 0.25s → 待写缓冲 + 20ms flush（掏空后挂起定时器）。
...
7. **launcher 缺依赖兜底 7 缺口(已修,/code-review 结论)**:①兜底写 启动失败.txt 无保护
   ...
   ⑦版本固定串 5 处拷贝无同步 → core/deps.py 单一来源,README 版本串由
...
- 2026-08-09 交互收敛（用户要求）：双击时间轴分割移除（分割只走工具栏"分割"按钮）；Delete 键删除补丁移除（补丁/复制/分段删除只走右栏删除按钮）；中键平移移除（平移只走选择模式放大后左键拖空白处）。新增用户文档（后并入 README）
